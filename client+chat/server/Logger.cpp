...
Logger::Logger(const char* prefix) :_log_pref(prefix){
...
auto Logger::_get_file_name() -> const std::string {
	std::string fname{ _log_pref };
	fname += '_'; fname += _log_fn;
	return fname;
}

auto Logger::read_line(std::string& l, int idx) -> bool {
	l.clear();
	_mtxs.lock_shared();
	if (!_file.is_open())
...
auto Logger::write(const char* line) -> bool {
