...
auto Message::show_unpacked(const char* msg,std::string& message_id) -> void {
	std::string tmp{ msg };
...
void Message::set_recv(const std::string& usr) {
	reciever = usr;
}

void Message::set_sender(const std::string& usr) {
	sender = usr;
}

void Message::set_msg_read() {
	type = eOld;
}
